...
int main(int argc, char **argv) {
        ...
        if (pid == 0) {
                ...
                for (int i = 0; i < N; i++) {
                        // This read will be successful only after parent write
                        // into the pipe and unblock the read action
                ...
                // p1 is for child; p2 is for parent
                // 0 is read      ; 1  is write
                //		exit(0);
        } else {
                // we are in parent
                close(p2[0]); // close parent read
                close(p1[1]); // close child write
